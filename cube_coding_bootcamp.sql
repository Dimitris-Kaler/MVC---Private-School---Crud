...
CONSTRAINT courses_per_students_courses_id__courses_id FOREIGN KEY (`courses_id`) REFERENCES `courses`(`id`) ON DELETE CASCADE ON UPDATE CASCADE ,
CONSTRAINT courses_per_students_students_id__students_id FOREIGN KEY (`students_id`) REFERENCES `students`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CONSTRAINT courses_per_trainers_courses_id__courses_id FOREIGN KEY (`courses_id`) REFERENCES `courses`(`id`) ON DELETE CASCADE ON UPDATE CASCADE ,
CONSTRAINT courses_per_trainers_trainers_id__trainers_id FOREIGN KEY (`trainers_id`) REFERENCES `trainers`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CONSTRAINT `students_per_assignments_student_id__students_id` FOREIGN KEY (`student_id`) REFERENCES `students`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT `students_per_assignments_assignment_id__assignments_id` FOREIGN KEY(`assignment_id`) REFERENCES `assignments`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT `students_per_assignments_course_id__courses_id` FOREIGN KEY(`course_id`) REFERENCES `courses`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
...
INSERT INTO customers(first_name,last_name,telephone,description) VALUES("George","Best",6984567332,"Can i delay the payment"),("Diego","Maradona",6453329761,"Lorem ispum dot"),
("Lotthar","Matthaus",6753457882,"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat");
